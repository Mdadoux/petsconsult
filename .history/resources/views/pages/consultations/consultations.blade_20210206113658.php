...
<div class="container-fluid">
    ...
    <p class="mb-4">
        {{__("La liste de toute les consultations")}}{{-- <a target="_blank" href="https://datatables.net">official DataTables documentation</a> --}}.
    </p>

    <div class="ml-auto w-25 text-right mb-3">
        <a href="{{route('consultations.create',Auth::user()->id)}}"
            ...
            <div class="{{--table-responsive--}}">
                ...
                <table class="table table-bordered dataTable-consultations" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        @php
                        $animalProp=ConsultationsController::getProprietaireById($consultation->animal['proprietaire_id'])
                        @endphp
                        <tr>
                            <td>
                                {{$consultation->id}}
                            ...
                            <td>
                                @if(!empty($animalProp->nom)) {{$animalProp->nom}} {{$animalProp->prenom}} @else <span
                                    class="badge badge-warning">{{__("Non communiqué")}}</span> @endif
                            </td>
                            <td><a href="patients/{{$consultation->animal['id']}}">{{$consultation->animal['nom']}}</a>
                            </td>
                            <td>{{$consultation->created_at->format('j F , Y')}}</td>
                            <td>
                                <div class="action-wrapper">
                                    <a href="{{route('consultations.show', ['Auth::user()->id','$consultation->id'])}}"
                                        class="btn btn-default btn-circle btn-sm">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="#" class="btn btn-danger btn-circle btn-sm delete-btn"
                                        data-titre="{{$consultation->titre}}" id="{{$consultation->id}}"
                                        data-toggle="modal" data-target="#delete-modal"
                                        data-form-route="{{route('consultations.destroy',['Auth::user()->id','consultation'])}}">
